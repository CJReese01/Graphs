...
public:
    ...
    int dijkstra(int start, int end){
        ...
        while(another){
            ...
            for(int i=0; i<MAXCONNECTIONS; i++){
                ...
                if(tempDist < dist[currentNode->connected[i]->index]){
                    ...
                    std::cout<<currentNode->name<<currentNode->connected[i]->name << " ";
                }
            }
            ...
            if(lastNode[currentNode->index] != NULL)
                std::cout<< " " << lastNode[currentNode->index]->name;
            currentNode = closest;
            another = false;
            for(int i=0; i<nodeIndex;i++){
                if(!visited[i]){
        ...
        for (int j = 0; j < nodeIndex; j++) {
            std::cout << "The distance from " << nodes[start]->name << " to " << nodes[j]->name << " is " << dist[j] << std::endl;
        }
        return dist[end];
    }
